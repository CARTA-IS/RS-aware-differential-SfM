...
namespace optflow
{
CustomOpticalFlowDeepFlow::CustomOpticalFlowDeepFlow()
{
    // parameters
    sigma = 0.6f;
    minSize = 25;
...
std::vector<Mat> CustomOpticalFlowDeepFlow::buildPyramid( const Mat& src )
{
...
void CustomOpticalFlowDeepFlow::calc( InputArray _I0, InputArray _I1, InputOutputArray _flow )
{
    // std::cout << "test parameters: sigma " << sigma << " minSize " << minSize << std::endl;
...
void CustomOpticalFlowDeepFlow::collectGarbage() {}

Ptr<DenseOpticalFlow> createOptFlow_CustomDeepFlow() {
    return makePtr<CustomOpticalFlowDeepFlow>();
}

Ptr<CustomOpticalFlowDeepFlow> createCustomDeepFlow() {
    return makePtr<CustomOpticalFlowDeepFlow>();
}
